...
*/
/*}}}*/
const LIB_UTIL_JS_ID        = "lib_util_js";
const LIB_UTIL_JS_TAG       = LIB_UTIL_JS_ID  +" (210706:16h:46)";  // eslint-disable-line no-unused-vars
...
if( log_this) lib_log.log("%c"+caller+"%c"+e.type+" on "+get_id_or_tag((e.path ? e.path[0] : e.target)),lbb+lbH,lbb+lb4);
/*}}}*/
...
//console.log("...el_array["+i+"]=["+get_id_or_tag( el_array[i] )+"]")
        ...
        if( is_el_child_of_el(e_target, el_array[i]) )
            return el_array[i];
    }

...
const LIB_POSTGRES_JS_TAG = LIB_POSTGRES_JS_ID+" (220829:19h:08)";    // eslint-disable-line no-unused-vars
/*}}}*/
...
{
let log_this=true;//FIXME
if( log_this ) console.log("new_Client: HOST=["+config.HOST+"] USER=["+config.USER+"] PASSWORD=["+config.PASSWORD+"] DATABASE=["+config.DATABASE+"]");
    if( !client )
    {
...
if(config.LOG_MORE)
log_M("  ┌────────────────────────────────────────────────────────────────────────────┐\n"
    + "… │ SQL "+count + sql.replace(/^/gm,"… │  ").replace(/ *\b(values|WHERE|AND|OR)\b/igm,"\n… │  $1")+"\n"
    + "  └────────────────────────────────────────────────────────────────────────────┘");
...
//console.dir(res.rows)
        ...
        let consumed_by
            = args.file_name && server_sql.populate_JSON(res, args, response);
...
log_M("dump_TABLES_next(db_name=["+db_name+"])");

...
let show_top_menu = function()
{
    ...
    if(!show_top_menu_timer )
    /**/show_top_menu_timer =   setTimeout(show_top_menu_handler, FIRST_VISIBILITY_DELAY);
};
/*}}}*/
/*_ show_top_menu_handler {{{*/
let show_top_menu_handler = function()
{
...
//log( "taxo_id_array:",taxo_id_array)
    ...
    show_taxo_id_array( taxo_id_array );
...
/*}}}*/
/*_ show_taxo_id_array {{{*/
let show_taxo_id_array = function(taxo_id_array)
{
...
/*}}}*/
/*_ show_taxo_id_menu {{{*/
let show_taxo_id_menu = function(taxo_id,clicked_EL)
{
/*{{{*/
    let caller = "show_taxo_id_menu";
let log_this = options.LOG1_STEP;

/*}}}*/
if( log_this) log("%c show_taxo_id_menu("+taxo_id+")", lbR+lf7);
...
{
    let visible_menu_EL_array = get_visible_menu_EL_array();
...
/*{{{*/
    ...
    // └──────────────────────┘
/*_ class_taxo_id_path_CSS {{{*/
let class_taxo_id_path_CSS = function(taxo_id_CSV,className)
{
/*{{{*/
let caller = "class_taxo_id_path_CSS";
let log_this = options.LOG1_STEP;
...
{
    class_taxo_id_path_CSS(taxo_id_path_CSV, CSS_SELECTED);
};
/*}}}*/
...
{
    class_taxo_id_path_CSS(taxo_id_path_CSV, CSS_COLLECTED);
};
/*}}}*/
...
/*}}}*/
    ...
    // └──────────────────────┘
/*{{{*/
/*_ get_visible_menu_EL_array {{{*/
let get_visible_menu_EL_array = function()
{
...
if( log_this) log("%c "+caller+": i_next: "+(pods_cluster.i_next+1)+"/"+visible_menu_EL_array.length+"%c XY "+m_x+" "+m_y+"%c"+((pod.lead_pod ? pod.lead_pod.id+"➔":"NO lead_pod➔")+pod.id)
                  ,lbL                                                                               ,lbC                 ,lbR                                                             );
    ...
    let lead_pod
        = (pod.id == "menu_"+taxo_json.id)
        ?  moving_pod
        :  layout_cluster_get_menu_EL_lead_pod( pod )
        || moving_pod;
    ...
    if( lead_pod )
    {
        ...
        //  pod.title += "\n lead_pod: "+ lead_pod.id+" top=["+ (lead_pod.getBoundingClientRect().top).toFixed(0)+"]";
...
//  pod.style.opacity    = (show_top_menu_timer) ? "0" : "1";
    pod.style.opacity    = "1";
...
/*}}}*/
/*_ layout_cluster_get_menu_EL_lead_pod {{{*/
let layout_cluster_get_menu_EL_lead_pod = function(menu_EL)
{
    if(menu_EL.lead_pod              ) return menu_EL.lead_pod;
//  if(menu_EL.lead_pod == null      ) return null;
 ...
 //log("%c layout_cluster_get_menu_EL_lead_pod%c"+menu_EL.id+"%c ...return ["+(lead_pod ? lead_pod.id : "")+"]", lbL+lf0, lbR+lf5, lbH+lf6)
    return lead_pod;
};
/*}}}*/
...
if( log_this) log("%c"+caller+"("+state+(_caller ? (" , "+_caller) : "")+") %c multiple_choice_state=["+multiple_choice_state+"]"
    ...
    if(        !multiple_choice_state
        &&      taxo_pods.get_collected_taxo_id_path_array()
        .concat(taxo_pods.get_selected_taxo_id_path_array ())
        .length > 1
      )
        taxo_menu.sel_clear(_caller);
};
/*}}}*/
...
//log("sub_menu_EL_is_visible:",sub_menu_EL_is_visible)
                    ...
                    /* SHOW [sub_menu_EL] {{{*/
                        ...
                        show_taxo_id_menu(menu_EL.id, menu_EL);
...
const MENU_ZINDEX_TOP = 2147483646; // @see: amazon.com
/*}}}*/
...
let get_taxo_id_path = function(taxo_id)
{
    ...
    while( parent_node && !taxo_id_path.startsWith( taxo_json.id ))
    {
...
/*}}}*/
    ...
    let       s_cleared = taxo_pods. unselect_menu_EL_all();

/*{{{*/
if( tag_this)
{
    let l_x = lfX[(s_cleared && 4) || (c_cleared && 5) || 9];
    log("%c"+caller+"("+_caller+") %cselected "+s_cleared+"%c collected "+c_cleared
        ,lbL+l_x                  ,lbC+lf4                ,lbC+lf5                 );
}
/*}}}*/
    /*}}}*/
...
/*}}}*/
    /*}}}*/
    if( nothing_cleared ) {
        /* ==> ITEMS SHOWN ➔ [collected]+[selected] {{{*/
        let            shadow_root = get_shadow_root();
        let el_array = shadow_root.querySelectorAll(taxo_pods.SELECTOR_BUTTONS_POD+":not("+taxo_pods.SELECTOR_HIDDEN+")");
        let  m_count = el_array.length;
        /**/el_array.forEach((el) => m_count -= (el.style.visibility == "hidden") ? 1 : 0); // skip invisible menus
        /**/el_array.forEach((el) => m_count -= (el.style.opacity    == "0"     ) ? 1 : 0); // skip faded     menus

        /*}}}*/
        /* 2/5 ITEMS  > 1  ➔ SHOWING ONLY TOP MENU {{{*/
        if(m_count > 1)
        {
if( tag_this)
    log("%c"+caller+"("+_caller+")%c SHOWING ONLY TOP MENU (instead of "+m_count+" out of "+el_array.length+")"
        ...
        /*}}}*/
        /* 3/5 ITEMS  > 0  ➔ UNLOADING ALL TAXO MENUS {{{*/
        else if(m_count > 0)
        {
if( tag_this)
    log("%c"+caller+"("+_caller+")%c UNLOADING ALL "+el_array.length+" TAXO MENUS"
        ...
        /*}}}*/
        /* 4/5 CACHE != 0  ➔ RELOADING PAGE SELECTION FROM CACHE {{{*/
...
if( tag_this)
    log("%c"+caller+"("+_caller+")%c RELOADING PAGE SELECTION FROM CACHE"
        ,lbL+lf9                 ,lbR+lf7);
            ...
            if(         taxo_pods.get_collected_taxo_id_path_array()
                .concat(taxo_pods.get_selected_taxo_id_path_array ())
                .length > 1
              ) {

                taxo_pods.set_multiple_choice_state( true );
            }
        }
        /*}}}*/
        /* 5/5 CACHE == 0  ➔ CANNOT RELOAD PAGE SELECTION (CACHE IS EMPTY {{{*/
        else {
if( tag_this)
...
/*➔ check_taxo_json {{{*/
/*{{{*/
const CHECK_TAXO_JSON_NEXT_ID_DELAY = 1000;
const CHECK_TAXO_JSON_CLEANUP_DELAY = 2000;
...
console.clear();
    ...
    {
        taxo_check_button.innerText = "Nothing\nto\ncheck";

        setTimeout(check_taxo_json_cleanup, CHECK_TAXO_JSON_CLEANUP_DELAY);
        return;
    }

    /*}}}*/
    /* DISPLAY [sample_id_array] {{{*/
    add_el_class(taxo_check_button.parentElement, "checking");
...
//if(check_idx == 0) log("%c➔ sample_id_array:", lf6, sample_id_array);
            ...
            if(check_idx < sample_id_array.length)
            {
                ...
                let     msg = "CHECKING "+(check_idx+1)+" / "+sample_id_array.length
                    +         "\n\n"
                    +         taxo_id.replace(/\./g,"\n")
                ;
//log("%c➔ "+msg, lf6)
                taxo_check_button.innerText = msg;

                /* [taxo_id] */
                if( taxo_id.indexOf(".") >= 0) {
...
let check_taxo_json_cleanup = function()
{
    /* RESTORE [taxo_check_button.textContent] */
    let       shadow_root = get_shadow_root();
    let taxo_check_button = shadow_root.getElementById( "taxo_check" );
    del_el_class(taxo_check_button.parentElement      , "checking"   );
    ...
    let el_array = shadow_root.querySelectorAll( ".checking");
    /**/el_array.forEach((el) => del_el_class(el, "checking")); // remove invisible menus
...
return { SHADOW_HOST_ID
    ...
    ,  show_taxo_id_array               : taxo_pods.show_taxo_id_array
    ...
    ,  class_taxo_id_path_CSS             : taxo_pods.class_taxo_id_path_CSS
    ...
    ,  get_selected_EL_array            : taxo_menu.get_selected_EL_array
...
/get_selected_taxo_id_path_array.*function
/class_taxo_id_path_CSS.*function
